...
the downloadable catalog contains ten apps across productivity, utilities, system, and games categories: notes, todo, clock & timer, disk usage, calendar, component browser, the inferno bfs1h demo, chicken run 3d, garden snake, and neon pong. downloadable apps remain catalog-only and are not included in the standalone installer image.
...
garden snake uses wasd/arrows or a touch relative to the snake's head; collect stars without striking the border or yourself, then press or tap to restart. neon pong uses w/s or up/down, or direct vertical touch, and races an adaptive cpu to seven. both games cap updates with timed event pulls and pause active play input while unfocused. short, rate-limited `computer.beep` cues mark catches, food, shots, hits, paddle returns, scores, wins, and losses without running sound work every frame. all three games use generated cell art and original code with no third-party game assets.
