...
if type(boot) ~= "table" and type(boot) ~= "userdata" then error("boot filesystem proxy was not supplied", 0) end
...
if gpu then
  local screenOk, bound = pcall(gpu.getScreen)
  if screen and (not screenOk or not bound) then pcall(gpu.bind, screen) end
...
local function paint(title, detail, progress)
  ...
  pcall(gpu.setBackground, 0x102a43)
  pcall(gpu.setForeground, 0xffffff)
  pcall(gpu.fill, 1, 1, width, height, " ")
  pcall(gpu.setBackground, 0x397fca)
  pcall(gpu.fill, 1, 1, width, 2, " ")
  pcall(gpu.set, 2, 1, "idk os recovery updater")
  pcall(gpu.setForeground, 0xbdd7ea)
  pcall(gpu.set, 2, 2, "running from ram - do not power off")
  pcall(gpu.setBackground, 0x102a43)
  pcall(gpu.setForeground, 0xffffff)
  pcall(gpu.set, 2, 5, tostring(title or "working"):sub(1, math.max(1, width - 3)))
  pcall(gpu.setForeground, 0x9fc4dd)
  pcall(gpu.set, 2, 7, tostring(detail or ""):sub(1, math.max(1, width - 3)))
  if type(progress) == "number" then
    local barWidth = math.max(10, width - 6)
    local filled = math.max(0, math.min(barWidth, math.floor(barWidth * progress)))
    pcall(gpu.setBackground, 0x173f5f)
    pcall(gpu.fill, 3, 10, barWidth, 1, " ")
    if filled > 0 then pcall(gpu.setBackground, 0x55d98b) pcall(gpu.fill, 3, 10, filled, 1, " ") end
    pcall(gpu.setBackground, 0x102a43)
  end
end
...
local function writeAtomic(path, data)
  ...
  if boot.exists(path) then
    ...
    if not removed and boot.exists(path) then removeTree(temp) return nil, removeReason end
  end
  local renamed, renameReason = boot.rename(temp, path)
  if not renamed then removeTree(temp) return nil, renameReason end
  return true
end

local internet = primary("internet")
if not internet then error("internet card is unavailable", 0) end

local function closeRequest(request)
  pcall(function()
    local close = request and request.close
    if type(close) == "function" then close() end
  end)
end

local function fetch(url, limit)
  local called, request, reason = pcall(internet.request, url)
  if not called then return nil, request end
  if not request then return nil, reason or "request failed" end
  local parts, size = {}, 0
  local lastData = rawComputer.uptime()
  while true do
    local readOk, chunk, readReason
    local methodOk, readMethod = pcall(function() return request.read end)
    if methodOk and type(readMethod) == "function" then readOk, chunk, readReason = pcall(readMethod)
    else readOk, chunk, readReason = pcall(function() return request() end) end
    if not readOk then closeRequest(request) return nil, chunk end
    if chunk == nil then
      if readReason then closeRequest(request) return nil, readReason end
      break
    elseif chunk == "" then
      if rawComputer.uptime() - lastData > 30 then closeRequest(request) return nil, "network timeout" end
      rawComputer.pullSignal(0.05)
    else
      lastData = rawComputer.uptime()
      size = size + #chunk
      if size > limit then closeRequest(request) return nil, "download exceeds size limit" end
      parts[#parts + 1] = chunk
    end
  end
  local responseCode
  pcall(function()
    ...
    if type(response) == "function" then responseCode = response() end
  end)
  closeRequest(request)
  if responseCode and (responseCode < 200 or responseCode >= 300) then return nil, "http " .. tostring(responseCode) end
...
local function safeTarget(path)
  if type(path) ~= "string" or #path > 160 or not path:match("^/[%w%._/-]+$") or
    path:find("..", 1, true) or path:find("//", 1, true) or path:sub(-1) == "/" then return false end
...
if not imageOk or type(image) ~= "table" or type(image.files) ~= "table" then error("invalid update manifest", 0) end
if #image.files < 1 or #image.files > 128 then error("invalid update file count", 0) end

local entries, seenTargets = {}, {}
for _, item in ipairs(image.files) do
  if type(item) ~= "table" or not safeSource(item.source) or not safeTarget(item.target) or seenTargets[item.target] then
...
if not cleanedStage then error("cannot clean staging directory: " .. tostring(cleanedStageReason), 0) end
local cleanedBackup, cleanedBackupReason = removeTree(BACKUP)
if not cleanedBackup then error("cannot clean backup directory: " .. tostring(cleanedBackupReason), 0) end
...
for index, item in ipairs(entries) do
  paint("downloading system files", string.format("%d/%d  %s", index, #entries, item.source), (index - 1) / #entries)
  local data, reason = fetch(BASE .. item.source, MAX_FILE)
  if not data then removeTree(STAGE) error(item.source .. ": " .. tostring(reason), 0) end
  total = total + #data
  if total > MAX_TOTAL then removeTree(STAGE) error("update exceeds total size limit", 0) end
  if item.source:sub(-4) == ".lua" then
    local chunk, syntaxError = load(data, "=" .. item.source, "t", {})
    if not chunk then removeTree(STAGE) error(item.source .. ": " .. tostring(syntaxError), 0) end
  end
  item.data = data
  local written, writeReason = writeFile(STAGE .. item.target, data)
  item.data = nil
  if not written then removeTree(STAGE) error("staging " .. item.target .. ": " .. tostring(writeReason), 0) end
end

local existed = {}
for index, item in ipairs(entries) do
  paint("backing up installed system", string.format("%d/%d  %s", index, #entries, item.target), index / #entries)
  existed[item.target] = boot.exists(item.target)
  if existed[item.target] then
    local old, reason = readFile(item.target)
    if not old then removeTree(STAGE) removeTree(BACKUP) error("backup " .. item.target .. ": " .. tostring(reason), 0) end
    local saved, saveReason = writeFile(BACKUP .. item.target, old)
    if not saved then removeTree(STAGE) removeTree(BACKUP) error("backup " .. item.target .. ": " .. tostring(saveReason), 0) end
  end
end
...
local applyError
for index, item in ipairs(entries) do
  paint("applying update", string.format("%d/%d  %s", index, #entries, item.target), index / #entries)
  applied[#applied + 1] = item.target
  local data, reason = readFile(STAGE .. item.target)
  if not data then applyError = "read staged " .. item.target .. ": " .. tostring(reason) break end
  local written, writeReason = writeAtomic(item.target, data)
  if not written then applyError = "write " .. item.target .. ": " .. tostring(writeReason) break end
end
...
if applyError then
  ...
  for index = #applied, 1, -1 do
    ...
    if existed[target] then
      ...
      if not old then rollbackErrors[#rollbackErrors + 1] = target .. ": " .. tostring(reason)
      else
        local restored, restoreReason = writeAtomic(target, old)
        if not restored then rollbackErrors[#rollbackErrors + 1] = target .. ": " .. tostring(restoreReason) end
      end
    else
      local removed, removeReason = removeTree(target)
      if not removed then rollbackErrors[#rollbackErrors + 1] = target .. ": " .. tostring(removeReason) end
    end
  end
  removeTree(STAGE)
  ...
  if #rollbackErrors > 0 then
    suffix = "; rollback errors: " .. table.concat(rollbackErrors, ", ") .. "; backup retained at " .. BACKUP
  else
...
removeTree(STAGE)
removeTree(BACKUP)
writeFile(RECOVERY .. "/last-update.log", string.format("updated %d files, %d bytes\n", #entries, total))
